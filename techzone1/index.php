...
<body>
<header>
  <a href="index.php" aria-label="Tech Zone Inicio">Tech Zone</a>
...
<main class="container">
  <section class="card" aria-labelledby="welcome-title">
    <h2 id="welcome-title">🚀 Bienvenidos a Tech Zone</h2>
    <p class="muted">Hardware premium y componentes de alto rendimiento. Explora nuestro catálogo e inicia sesión para comprar.</p>
  </section>

  <?php if(!$res || $res->num_rows === 0){ ?>
    <div class="card">
      <p>No hay artículos registrados en la base de datos.</p>
    </div>
  <?php } else { ?>
    <div class="grid">
      <?php while($p = $res->fetch_assoc()){ ?>
        <a href="producto.php?id=<?php echo (int)$p["id"]; ?>" class="product-link" aria-label="Detalles de <?php echo htmlspecialchars($p["nombre"]); ?>">
          <article class="card" style="height: 100%; margin-bottom:0;">
            ...
            <?php if($img !== "") { ?>
              <img class="producto-img" src="img/<?php echo rawurlencode($img); ?>" alt="Imagen de <?php echo htmlspecialchars($p["nombre"]); ?>">
            <?php } else { ?>
              <div class="producto-sin-img" aria-hidden="true">Sin imagen</div>
            <?php } ?>
